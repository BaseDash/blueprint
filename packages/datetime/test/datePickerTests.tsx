...
describe("<DatePicker>", () => {
    ...
    describe("reconciliates dayPickerProps", () => {
        ...
        it("enable out-of-range min dates if ignoreRange is true", () => {
            const defaultValue = new Date(2017, Months.SEPTEMBER, 1);
            const { getDay, clickPreviousMonth } = wrap(
                <DatePicker defaultValue={defaultValue} minDate={new Date(2017, Months.AUGUST, 20)} ignoreRange={true} />,
            );
            clickPreviousMonth();
            assertDayDisabled(getDay(10), false);
            assertDayDisabled(getDay(21), false);
        ...
        it("enable out-of-range max dates if ignoreRange is true", () => {
            const defaultValue = new Date(2017, Months.SEPTEMBER, 1);
            const { getDay } = wrap(
                <DatePicker defaultValue={defaultValue} maxDate={new Date(2017, Months.SEPTEMBER, 20)} ignoreRange={true} />,
            );
            assertDayDisabled(getDay(10), false);
        });
